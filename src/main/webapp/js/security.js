			...
			user.$refresh().then(
				function() {
					...
					if (user.$isAuthenticated() && $scope.remember) {
						...
						$cookies.put('BookStore_Example_user', user.user, opt);
						$cookies.put('BookStore_Example_pass', hash, opt);
					}
...
function($scope, $location, $cookies, UserProfile, $http) {
	...
	$cookies.remove('BookStore_Example_user');
	$cookies.remove('BookStore_Example_pass');
...
function($q, User, $cookies, $http) {
	...
	var fetchUserProfile = function() {
		...
		//IF HAS COOKIES SET 
		if ($cookies.get('BookStore_Example_user')) {
			$http.defaults.headers.common['user'] = $cookies.get('BookStore_Example_user');
			$http.defaults.headers.common['pass'] = $cookies.get('BookStore_Example_pass');
		};
